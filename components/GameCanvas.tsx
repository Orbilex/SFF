...
import { playSound, playBetterBuddyTheme, stopBetterBuddyTheme, startBetterBuddyShootLoop, stopBetterBuddyShootLoop } from '../services/audio';
...
interface GameCanvasProps {
  ...
  onQuantumUsed?: () => void;
  onReturnToMenu?: () => void;
  selectedServoForPlacement?: any | null;
  onPlaceServo?: () => void;
...
const GameCanvas: React.FC<GameCanvasProps> = ({ 
  ...
  useEffect(() => {
    if (forceActivateSkillId && servosRef.current.length > 0) {
      const servo = servosRef.current[0];
      const skill = servo.data.skills.find((s: any) => s.id === forceActivateSkillId);
      if (skill && skill.currentLevel > 0) {
        // Force activate
        activateServoSkill(servo, skill, true);
        if (onSkillActivated) onSkillActivated();
      }
    }
  }, [forceActivateSkillId]);
  ...
  const activateServoSkill = (servo: any, skill: any, force: boolean = false) => {
    ...
    if (skill.id === 'loyal_buddy') {
      ...
      servo.castingEndTime = gameTimeRef.current + 2000; // 2 seconds casting
      servo.castingSkillData = skill;
      playSound('BETTER_BUDDY_CONSTRUCT');
    }
  };
  ...
  // Update placed servo data when activeServo changes (e.g. from Arsenal upgrades)
  useEffect(() => {
    if (activeServo && servosRef.current.length > 0) {
      servosRef.current.forEach(s => {
        if (s.id === activeServo.id) {
          s.data = activeServo;
        }
      });
    }
  ...
  const handleCanvasClick = (e: React.MouseEvent) => {
    ...
    if (!selectedTower && !selectedServoForPlacement) {
    ...
    if (selectedServoForPlacement) {
      // Check 5x5 area for collisions
      const halfSize = Math.floor(5 / 2) * GRID_SIZE;
      let collision = false;
      ...
      pathsRef.current.forEach(path => {
        for (let i = 0; i < path.length - 1; i++) {
            ...
            if (
              gridX + halfSize > minX && gridX - halfSize < maxX &&
              gridY + halfSize > minY && gridY - halfSize < maxY
            ) {
              collision = true;
            }
        }
      ...
      collision = towersRef.current.some(t => 
        Math.abs(t.position.x - gridX) <= halfSize && Math.abs(t.position.y - gridY) <= halfSize
      );

      if (collision) return;

      // Check servo collision
      collision = servosRef.current.some(s => 
        Math.abs(s.position.x - gridX) <= halfSize * 2 && Math.abs(s.position.y - gridY) <= halfSize * 2
      );

      if (collision) return;

      servosRef.current.push({
        id: selectedServoForPlacement.id,
        type: selectedServoForPlacement.type,
        position: { x: gridX, y: gridY },
        lastFired: gameTimeRef.current,
        targetId: null,
        angle: 0,
        data: selectedServoForPlacement,
        activeMinions: []
      });

      playSound('BUILD');
      spawnParticles(gridX, gridY, '#9333ea', 30, 'SPARK');
      if (onPlaceServo) onPlaceServo();
      return;
    }
  ...
  const update = (gameTime: number, dt: number) => {
    ...
    // Update Servos and Minions
    servosRef.current.forEach(servo => {
      // Servo Attack Logic (Infinite Range)
      const stats = SERVO_STATS[servo.type as ServoType];
      const damage = stats.damage * servo.data.stats.damageMult * (1 + (servo.data.damageLevel || 0) * 0.2);
      const cooldown = stats.cooldown / servo.data.stats.fireRateMult;

      if (servo.castingSkill) {
        if (gameTime >= servo.castingEndTime) {
          // Finish casting
          if (servo.castingSkill === 'loyal_buddy') {
            ...
            servo.lastLoyalBuddySpawnTime = gameTime;
            spawnParticles(servo.position.x, spawnY, '#9333ea', 20, 'SPARK');
            ...
            servo.lastBetterBuddySpawnTime = gameTime;
            playSound('BETTER_BUDDY_SPAWN');
            playBetterBuddyTheme();
            spawnParticles(servo.position.x, spawnY, '#d8b4fe', 50, 'SPARK');
          }
          servo.castingSkill = null;
          servo.castingSkillData = null;
        }
      } else if (gameTime - servo.lastFired >= cooldown) {
        if (enemiesRef.current.length > 0) {
          // Target the enemy furthest along the path
          const target = [...enemiesRef.current].sort((a, b) => b.distanceTraveled - a.distanceTraveled)[0];
          
          const dx = target.position.x - servo.position.x;
          const dy = target.position.y - servo.position.y;
          const aimAngle = Math.atan2(dy, dx);
          servo.angle = aimAngle;
          
          const isFacingLeft = Math.abs(aimAngle) > Math.PI / 2;
          const hover = Math.sin(gameTime * 0.003) * 1.5 - 5;
          const mechBreathe = Math.sin(gameTime * 0.004) * 0.5;
          
          const shoulderX = (isFacingLeft ? -14 : 14) * 3.2;
          const shoulderY = (-12 - mechBreathe + hover) * 3.2;
          
          const yOffset = isFacingLeft ? -3.5 : 3.5;
          const armX = (54 * Math.cos(aimAngle) + yOffset * Math.sin(aimAngle)) * 3.2;
          const armY = (54 * Math.sin(aimAngle) - yOffset * Math.cos(aimAngle)) * 3.2;
          
          const muzzleX = servo.position.x + shoulderX + armX;
          const muzzleY = servo.position.y + shoulderY + armY;
          ...
          servo.lastFired = gameTime;
          playSound('BUILDER_SHOOT');
          spawnParticles(muzzleX, muzzleY, stats.color, 8, 'SPARK');
          spawnParticles(muzzleX, muzzleY, '#ffffff', 3, 'SMOKE');
          
          const ejectX = servo.position.x + shoulderX + (30 * Math.cos(aimAngle) + yOffset * Math.sin(aimAngle)) * 3.2;
          const ejectY = servo.position.y + shoulderY + (30 * Math.sin(aimAngle) - yOffset * Math.cos(aimAngle)) * 3.2;
          spawnParticles(ejectX, ejectY, '#fbbf24', 1, 'SHELL', undefined, aimAngle);
        }
      }

      // Randomly activate skills
      if (Math.random() < 0.001 * dt) {
        const unlockedSkills = servo.data.skills.filter((s: any) => s.currentLevel > 0);
      ...
      // Update Minions
      servo.activeMinions = servo.activeMinions.filter((minion: any) => {
        const age = gameTime - minion.spawnTime;
        
        if (minion.type === 'LOYAL_BUDDY') {
          if (age >= minion.lifespan - 2000 && !minion.hasPulsed) {
            minion.hasPulsed = true;
            playSound('LOYAL_BUDDY_PULSE');
          }
        } else if (minion.type === 'BETTER_BUDDY') {
          if (age >= minion.lifespan - 4000 && !minion.isCurledUp) {
            minion.isCurledUp = true;
            playSound('BETTER_BUDDY_EXPIRE');
            stopBetterBuddyShootLoop();
          }
          if (!minion.isCurledUp && Math.random() < 0.0005 * (dt / 16)) {
            playSound('BETTER_BUDDY_NOISE');
          }
        }

        if (age >= minion.lifespan) {
          // Explode
          const explosionRadius = minion.type === 'BETTER_BUDDY' ? 10000 : 200; // Map wide for better buddy, increased for loyal buddy
          
          if (minion.type === 'LOYAL_BUDDY') {
            playSound('LOYAL_BUDDY_EXPLOSION');
            // High quality blast effect
            spawnExplosion(minion.position.x, minion.position.y, '#facc15', explosionRadius);
            spawnParticles(minion.position.x, minion.position.y, '#facc15', 30, 'SPARK');
            spawnParticles(minion.position.x, minion.position.y, '#ffffff', 10, 'CLOUD');
            // Blast radius ring
            projectilesRef.current.push({
              id: Math.random().toString(36),
              ...
              angle: 0,
              damage: 0,
              type: 'EM_SLASH', // Reusing EM_SLASH for a cool expanding ring effect
              targetId: null,
              color: '#facc15',
              radius: explosionRadius,
              timer: 200 // Used for progress
            });
          } else if (minion.type === 'BETTER_BUDDY') {
            playSound('BETTER_BUDDY_EXPLOSION');
            stopBetterBuddyTheme();
            stopBetterBuddyShootLoop();
            screenFlashRef.current = 1.0; // White flash
            spawnExplosion(minion.position.x, minion.position.y, '#d8b4fe', explosionRadius);
            // Nuclear mushroom cloud effect
            spawnParticles(minion.position.x, minion.position.y, '#d8b4fe', 100, 'CLOUD');
            spawnParticles(minion.position.x, minion.position.y - 100, '#ffffff', 50, 'CLOUD');
            spawnParticles(minion.position.x, minion.position.y, '#9333ea', 100, 'SPARK');
          } else {
            spawnParticles(minion.position.x, minion.position.y, '#ef4444', 50, 'SPARK');
            playSound('EXPLOSION');
          }
          
          enemiesRef.current.forEach(e => {
            if (getDist(e.position, minion.position) <= explosionRadius) {
              const dmgMult = minion.type === 'LOYAL_BUDDY' ? 100 : (minion.type === 'BETTER_BUDDY' ? 500 : 5); // Massively increased explosion damage
              e.hp -= minion.damage * dmgMult;
              e.damageFlashTimer = 100;
              spawnParticles(e.position.x, e.position.y - 10, '#ffffff', 1, 'TEXT', `-${Math.floor(minion.damage * dmgMult)}`);
            }
          });
          return false;
        }
        ...
        enemiesRef.current.forEach(e => {
          const d = getDist(e.position, minion.position);
          if (d < minDist) {
            minDist = d;
            nearestEnemy = e;
          }
        ...
        if (nearestEnemy && (!minion.isCurledUp)) {
          ...
          const isBetter = minion.type === 'BETTER_BUDDY';
          const targetDist = isBetter ? 250 : 100;
          
          // Move towards or away to maintain distance
          if (dist > targetDist + 10) {
            minion.position.x += (dx / dist) * minion.speed * (dt / 16);
            minion.position.y += (dy / dist) * minion.speed * (dt / 16);
          ...
          // Attack
          if (minion.type === 'LOYAL_BUDDY') {
            if (minion.isReloading) {
              if (gameTime >= minion.reloadEndTime) {
                minion.isReloading = false;
                minion.bulletsFired = 0;
              }
            } else {
              const fireRate = 1000 / 16; // 16 bullets in 1 second
              if (gameTime - (minion.lastFired || 0) >= fireRate) {
                minion.lastFired = gameTime;
                ...
                projectilesRef.current.push({
                  ...
                  type: 'LOYAL_BUDDY_BULLET',
                  targetId: target.id,
                  color: '#facc15',
                  radius: 2
                });
                
                if (minion.bulletsFired === 0) {
                  playSound('LOYAL_BUDDY_SHOOT');
                }
                
                // Reduce particles to save performance
                if (Math.random() < 0.2) {
                  spawnParticles(minion.position.x, minion.position.y, '#facc15', 1, 'SPARK');
                }
                
                minion.bulletsFired++;
                if (minion.bulletsFired >= 16) {
                  minion.isReloading = true;
                  minion.reloadEndTime = gameTime + 2000;
                  playSound('LOYAL_BUDDY_RELOAD');
                }
              }
            }
          } else if (minion.type === 'BETTER_BUDDY') {
            ...
            const coneLength = 600;
            if (dist <= coneLength) {
                minion.lastFired = gameTime;
                
                // Start shooting loop sound if not already playing
                startBetterBuddyShootLoop();
                
                // Deal AoE damage in a cone
                const coneAngle = Math.PI / 6; // 30 degrees spread
                
                enemiesRef.current.forEach(e => {
                  const edx = e.position.x - minion.position.x;
                  const edy = e.position.y - minion.position.y;
                  ...
                  if (edist < coneLength) {
                    ...
                    if (angleDiff < coneAngle) {
                      e.hp -= minion.damage * (dt / 16); // Damage per frame
          ...
          // If no enemies nearby, stop shooting sound
          stopBetterBuddyShootLoop();
        }
  ...
  const draw = (gameTime: number) => {
    ...
    const renderList = [
        ...
        ...servosRef.current.map(s => ({ type: 'SERVO', y: s.position.y, obj: s })),
        ...servosRef.current.flatMap(s => s.activeMinions.map((m: any) => ({ type: 'MINION', y: m.position.y, obj: m }))),
    ...
    renderList.forEach(item => {
        ...
        else if (item.type === 'SERVO') drawServo3D(ctx, item.obj as any, time);
        else if (item.type === 'MINION') drawMinion3D(ctx, item.obj as any, time);
        else {
    ...
    projectilesRef.current.forEach(proj => {
        ...
        if (proj.type === 'EM_SNIPER') {
             ...
             ctx.shadowColor = proj.color || '#ef4444'; ctx.shadowBlur = 20;
             ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(0, 0, proj.radius || 40, 0, Math.PI * 2); ctx.stroke();
             ctx.strokeStyle = proj.color || '#ef4444'; ctx.lineWidth = 6; ctx.beginPath(); ctx.arc(0, 0, (proj.radius || 40) - 5, 0, Math.PI * 2); ctx.stroke();
            ...
            ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.roundRect(-12, -2, 24, 4, 2); ctx.fill();
            ctx.globalCompositeOperation = 'source-over'; ctx.shadowBlur = 0;
        } else if (proj.type === 'LOYAL_BUDDY_BULLET') {
            ctx.translate(proj.position.x, proj.position.y); ctx.rotate(proj.angle || 0);
  ...
  const drawMinion3D = (ctx: CanvasRenderingContext2D, minion: any, time: number) => {
      ...
      if (minion.type === 'LOYAL_BUDDY') {
          // Loyal Buddy: Bipedal, no head, torso gun barrel, Builder colors
          const scale = 1.5;
          ctx.scale(scale, scale);
          
          const walkCycle = Math.sin(time * 0.01 * minion.speed);
          const isShooting = time - (minion.lastFired || 0) < 100;
          const aimAngle = minion.angle || 0;
          const isFacingLeft = Math.abs(aimAngle) > Math.PI / 2;
          ...
          const cPurpleGlow = '#d8b4fe';
          const cYellow = '#facc15'; // For the pulse/bullets

          const drawPoly = (points: number[][], fill: string, stroke?: string) => {
              ctx.fillStyle = fill;
              ctx.beginPath();
          ...
          // Pulse effect
          if (minion.hasPulsed) {
              const pulseScale = 1 + Math.sin(time * 0.02) * 0.5;
              ctx.shadowColor = cYellow;
              ctx.shadowBlur = 30 * pulseScale;
              
              // Draw expanding warning rings
              ctx.strokeStyle = `rgba(250, 204, 21, ${1 - (time % 500) / 500})`;
              ctx.lineWidth = 2;
              ctx.beginPath();
              ctx.arc(0, 0, 10 + ((time % 500) / 500) * 30, 0, Math.PI * 2);
              ctx.stroke();
          } else {
              ctx.shadowColor = cPurpleGlow;
              ctx.shadowBlur = 10;
          }

          // Legs (Bipedal mechanical)
          // Back Leg
          ctx.save();
          ctx.translate(-4, 4);
          ctx.rotate(walkCycle * 0.5);
          drawPoly([[-2, 0], [2, 0], [3, 10], [-1, 10]], cArmorDark, cArmorHighlight);
          drawPoly([[-2, 10], [4, 10], [5, 12], [-3, 12]], cArmorMid, cArmorHighlight); // Foot
          ctx.restore();
          ...
          ctx.translate(4, 4);
          ctx.rotate(-walkCycle * 0.5);
          drawPoly([[-2, 0], [2, 0], [3, 10], [-1, 10]], cArmorMid, cArmorHighlight);
          drawPoly([[-2, 10], [4, 10], [5, 12], [-3, 12]], cArmorLight, cArmorHighlight); // Foot
          ctx.restore();

          // Torso (Main chassis)
          const hover = Math.sin(time * 0.005) * 1;
          ctx.translate(0, hover);
          
          drawPoly([[-8, -8], [8, -8], [10, 4], [-10, 4]], cArmorDark, cArmorHighlight);
          drawPoly([[-6, -6], [6, -6], [8, 2], [-8, 2]], cArmorMid, cArmorHighlight);
          
          // Core Glow
          ctx.fillStyle = minion.hasPulsed ? cYellow : cPurple;
          ctx.beginPath();
          ctx.arc(0, -2, 3, 0, Math.PI * 2);
          ctx.fill();

          // Torso Gun Barrel (Detailed)
          ctx.save();
          // Aiming rotation (adjusted for facing left)
          let gunAngle = isFacingLeft ? Math.PI - aimAngle : aimAngle;
          ctx.rotate(gunAngle);
          
          const recoil = isShooting ? -3 : 0;
          ctx.translate(recoil, 0);
          
          // Gun base
          drawPoly([[4, -4], [12, -3], [12, 3], [4, 4]], cArmorDark, cArmorHighlight);
          // Gun barrel
          drawPoly([[12, -2], [20, -2], [20, 2], [12, 2]], cArmorMid, cWhite);
          // Gun nozzle
          drawPoly([[20, -3], [24, -2], [24, 2], [20, 3]], cArmorLight, cWhite);
          
          // Energy line on barrel
          ctx.fillStyle = cPurpleGlow;
          ctx.fillRect(14, -0.5, 4, 1);
          ...
          // Giant Mechanical Whale
          const scale = 7.0; // 2x bigger
          ctx.scale(scale, scale);
          ...
          const swimCycle = Math.sin(time * 0.005);
          const breatheCycle = Math.sin(time * 0.002);
          ...
          if (minion.isCurledUp) {
              // Curled into a ball (nuclear core exposed)
              ctx.rotate(time * 0.01); // Spinning ball
              
              ctx.fillStyle = cArmorMid;
              ctx.strokeStyle = cPurpleGlow;
              ctx.lineWidth = 2;
              ...
              // Core pulsing
              const pulse = Math.abs(Math.sin(time * 0.02));
              applyGlow(20 + pulse * 20, cPurpleGlow);
              ...
              for (let i = 0; i < 4; i++) {
                  ...
                  drawPoly([[15, -10], [25, 0], [15, 10], [20, 0]], cArmorLight, cArmorHighlight);
                  ctx.restore();
              }
          } else {
              // High-Tech Angular Robot Whale
              const isShooting = time - (minion.lastFired || 0) < 100;
              
              // Fluidity: multiple swim cycles with phase shifts
              const timeSec = time * 0.001;
              const swim1 = Math.sin(timeSec * 5);
              const swim2 = Math.sin(timeSec * 5 - 1);
              const swim3 = Math.sin(timeSec * 5 - 2);

              // Sonar Waves
              if (isShooting && !minion.isCurledUp) {
                  ctx.save();
                  ctx.translate(40, 4); // Mouth position
                  
                  const beamLength = 600 / 7; // ~85
                  const timeMs = time;
                  ...
                  // Sonar waves
                  for (let i = 0; i < 6; i++) {
                      const waveProgress = ((timeMs * 0.002 + i * 0.166) % 1);
                      const xPos = waveProgress * beamLength;
                      
                      const radiusY = 5 + waveProgress * 25; 
                      const radiusX = 2 + waveProgress * 8;  // Perspective width
                      
                      const waveAlpha = (1 - waveProgress) * Math.min(1, waveProgress * 8);
                      
                      // Deep purple outer distortion
                      ctx.strokeStyle = `rgba(147, 51, 234, ${waveAlpha * 0.6})`;
                      ctx.lineWidth = 10 + waveProgress * 20;
                      ctx.beginPath();
                      ctx.ellipse(xPos, 0, radiusX, radiusY, 0, 0, Math.PI * 2);
                      ctx.stroke();
                      
                      // Bright purple main ring
                      ctx.strokeStyle = `rgba(192, 132, 252, ${waveAlpha})`;
                      ctx.lineWidth = 3 + waveProgress * 5;
                      ctx.beginPath();
                      ctx.ellipse(xPos, 0, radiusX, radiusY, 0, 0, Math.PI * 2);
                      ctx.stroke();
                      
                      // Blinding white inner core ring
                      ctx.strokeStyle = `rgba(255, 255, 255, ${waveAlpha * 0.8})`;
                      ctx.lineWidth = 1 + waveProgress * 2;
                      ctx.beginPath();
                      ctx.ellipse(xPos, 0, radiusX, radiusY, 0, 0, Math.PI * 2);
                      ctx.stroke();
                  }
                  
                  // Central sonic distortion beam
                  ctx.fillStyle = `rgba(168, 85, 247, 0.15)`;
                  ctx.beginPath();
                  ctx.moveTo(0, -4);
                  ctx.lineTo(beamLength, -25 + Math.sin(timeMs * 0.02) * 5);
                  ctx.lineTo(beamLength, 25 + Math.cos(timeMs * 0.02) * 5);
                  ctx.lineTo(0, 4);
                  ctx.fill();
                  
                  // Inner core (mouth glow)
                  applyGlow(25, '#a855f7');
                  ctx.fillStyle = '#ffffff';
                  ctx.beginPath();
                  ctx.arc(0, 0, 6 + Math.sin(timeMs * 0.03) * 3, 0, Math.PI * 2);
                  ctx.fill();
              ...
              // Tail Fin
              ctx.save();
              ctx.translate(-35, swim3 * 2);
              ctx.rotate(swim3 * 0.4);
              // Upper tail lobe
              drawPoly([[0, -2], [-10, -15], [-18, -18], [-8, -4]], cArmorDark, cArmorHighlight);
              // Lower tail lobe
              drawPoly([[0, 2], [-8, 15], [-15, 18], [-6, 4]], cArmorDark, cArmorHighlight);
              // Tail center connector
              drawPoly([[2, -3], [-4, 0], [2, 3], [6, 0]], cArmorMid, cPurple);
              ctx.restore();

              // Tail section 1
              ctx.save();
              ctx.translate(-25, swim2 * 1.5);
              ctx.rotate(swim2 * 0.2);
              drawPoly([[-10, -5], [0, -7], [0, 7], [-10, 5]], cArmorMid, cArmorHighlight);
              drawPoly([[-5, -2], [0, -2], [0, 2], [-5, 2]], cArmorDark, cPurple); // Energy seam
              ctx.restore();

              // Main Body Group
              ctx.save();
              ctx.translate(0, swim1 * 1);
              ctx.rotate(swim1 * 0.05);

              // Rear Body
              drawPoly([[-25, -7], [-10, -10], [-10, 10], [-25, 7]], cArmorLight, cArmorHighlight);

              // Mid Body
              drawPoly([[-10, -10], [15, -12], [15, 12], [-10, 10]], cArmorMid, cArmorHighlight);
              
              // Side Armor Plating (adds 3D depth)
              drawPoly([[-5, -2], [10, -3], [12, 5], [-8, 4]], cArmorDark, cArmorHighlight);
              
              // Energy Core / Vents on side
              applyGlow(10, cPurpleGlow);
              drawPoly([[0, 0], [6, 0], [7, 2], [-1, 2]], cPurple, cWhite);
              drawPoly([[-6, 0], [-2, 0], [-1, 2], [-7, 2]], cPurple, cWhite); // Extra vent
              clearGlow();

              // Dorsal Fin
              drawPoly([[0, -11], [8, -22], [15, -12]], cArmorDark, cArmorHighlight);
              // Dorsal glow
              applyGlow(10, cPurpleGlow);
              drawPoly([[4, -12], [8, -18], [11, -12]], cPurple);
              clearGlow();

              // Front Body
              drawPoly([[15, -12], [35, -10], [35, 6], [15, 12]], cArmorLight, cArmorHighlight);

              // Head / Upper Jaw
              drawPoly([[35, -10], [50, -4], [50, 2], [35, 2]], cArmorMid, cArmorHighlight);
              // Snout tip
              drawPoly([[50, -4], [55, -2], [55, 2], [50, 2]], cArmorDark, cArmorHighlight);

              // Visor (High-tech glowing slit)
              applyGlow(15, cPurpleGlow);
              drawPoly([[38, -6], [48, -3], [48, -1], [38, -3]], cPurpleGlow, cWhite);
              // Extra eye detail
              ctx.fillStyle = cWhite;
              ctx.beginPath(); ctx.arc(45, -2.5, 1, 0, Math.PI*2); ctx.fill();
              clearGlow();
              ...
              ctx.translate(30, 4);
              // Smooth jaw opening
              const jawOpen = isShooting ? Math.PI / 4 : (Math.sin(timeSec * 10) * 0.05 + 0.05);
              ctx.rotate(jawOpen);
              drawPoly([[0, 0], [5, -2], [22, -2], [20, 4], [0, 6]], cArmorDark, cArmorHighlight);
              // Jaw teeth/energy emitters
              applyGlow(10, cPurpleGlow);
              drawPoly([[5, -2], [8, -4], [10, -2]], cPurple);
              drawPoly([[12, -2], [15, -4], [17, -2]], cPurple);
              
              // Inner mouth glow when shooting
              if (isShooting) {
                  applyGlow(20, cPurpleGlow);
                  ctx.fillStyle = cWhite;
                  ctx.beginPath();
                  ctx.arc(10, -6, 4 + Math.random() * 2, 0, Math.PI * 2);
                  ctx.fill();
              }
              clearGlow();
              ctx.restore();
              ...
              ctx.save();
              ctx.translate(10, 8);
              ctx.rotate(-swim1 * 0.3);
              drawPoly([[0, 0], [-8, 20], [2, 18], [8, 0]], cArmorDark, cArmorHighlight);
              // Fin accent
              applyGlow(5, cPurpleGlow);
              drawPoly([[-2, 4], [-4, 14], [0, 12]], cPurple);
              clearGlow();
              ctx.restore();
              
              ctx.restore(); // End Main Body Group
          }
      }
      ...
      // HP Bar
      if (minion.type !== 'LOYAL_BUDDY' && minion.type !== 'BETTER_BUDDY') {
        const hpPct = Math.max(0, minion.hp / minion.maxHp); 
        const barY = y - (isBetter ? 60 : 30); 
        const barW = isBetter ? 40 : 20; 
        ctx.fillStyle = '#111'; 
        ctx.fillRect(x - barW/2, barY, barW, 4); 
        ctx.fillStyle = isBetter ? '#ef4444' : '#fbbf24'; 
  ...
  const drawServo3D = (ctx: CanvasRenderingContext2D, servo: any, time: number) => {
      ...
      if (servo.targetId) {
          const target = enemiesRef.current.find(e => e.id === servo.targetId);
          if (target) {
              aimAngle = Math.atan2(target.position.y - y, target.position.x - x);
          }
      }
      ...
      for (let i = 0; i < 3; i++) {
          ...
          // Left plate
          drawPoly([
              [-5, offsetY], [-spread, offsetY + 15], [-spread + 4, offsetY + 20], [-2, offsetY + 10]
          ], cArmorMid, cArmorHighlight);
      ...
      // Chest Plates (Broad)
      drawPoly([
          [0, 5], [-16, -10 - mechBreathe], [-10, -16 - mechBreathe], 
          [10, -16 - mechBreathe], [16, -10 - mechBreathe]
      ], cArmorMid, cArmorHighlight);
      
      // Inner Chest/Vest (White/Light Armor)
      drawPoly([
          [0, 2], [-10, -10 - mechBreathe], [-6, -14 - mechBreathe], 
          [6, -14 - mechBreathe], [10, -10 - mechBreathe]
      ], cWhite, cArmorLight);

      // Alien Reactor Core (Tie replacement)
      applyGlow(20);
      ctx.fillStyle = cPurpleGlow;
      ctx.beginPath();
      ...
      // Casting Animation Logic
      let castProgress = 0;
      let betterBuddyCastProgress = 0;
      if (servo.castingSkill === 'loyal_buddy') {
          const castDuration = 2000;
          const timeCasting = castDuration - (servo.castingEndTime - time);
          castProgress = Math.max(0, Math.min(1, timeCasting / castDuration));
      } else if (servo.castingSkill === 'better_buddy') {
          const castDuration = 2000;
          const timeCasting = castDuration - (servo.castingEndTime - time);
          betterBuddyCastProgress = Math.max(0, Math.min(1, timeCasting / castDuration));
          castProgress = betterBuddyCastProgress; // Make hat split open!
      }

      // High-quality construction animation
      if (betterBuddyCastProgress > 0) {
          ctx.save();
          ctx.translate(0, -80); // Above the builder
          
          const p = betterBuddyCastProgress; // 0 to 1
          const timeMs = time;
          
          // Holographic cylinder base
          const cylWidth = 70;
          
          ctx.globalCompositeOperation = 'screen';
          
          // Base projection from hat
          const projAlpha = Math.min(1, p * 2) * 0.4;
          const grad = ctx.createLinearGradient(0, 80, 0, -50);
          grad.addColorStop(0, `rgba(168, 85, 247, ${projAlpha})`);
          grad.addColorStop(1, `rgba(168, 85, 247, 0)`);
          
          ctx.fillStyle = grad;
          ctx.beginPath();
          ctx.moveTo(-10, 80); // from hat
          ctx.lineTo(10, 80);
          ctx.lineTo(cylWidth, -50);
          ctx.lineTo(-cylWidth, -50);
          ctx.fill();
          
          // Holographic scanning rings moving up
          for (let i = 0; i < 3; i++) {
              const ringP = (timeMs * 0.001 + i * 0.33) % 1;
              const ringY = 80 - ringP * 130;
              const ringW = 10 + ringP * (cylWidth - 10);
              
              ctx.strokeStyle = `rgba(216, 180, 254, ${(1 - ringP) * 0.5})`;
              ctx.lineWidth = 2;
              ctx.beginPath();
              ctx.ellipse(0, ringY, ringW, ringW * 0.2, 0, 0, Math.PI * 2);
              ctx.stroke();
          }
          
          ctx.globalCompositeOperation = 'source-over';
          
          // Draw the whale being constructed
          ctx.save();
          ctx.scale(7.0 / 3.2, 7.0 / 3.2); // Adjust for builder's 3.2x scale
          ...
          const drawPoly = (points: number[][], fill: string, stroke?: string, lineW: number = 0.5, isWireframe: boolean = false) => {
              ctx.beginPath();
              ctx.moveTo(points[0][0], points[0][1]);
              for(let i=1; i<points.length; i++) ctx.lineTo(points[i][0], points[i][1]);
              ctx.closePath();
          ...
          const drawWhaleParts = (isWireframe: boolean) => {
              drawPoly([[-25, -7], [-10, -10], [-10, 10], [-25, 7]], cArmorLight, cArmorHighlight, 0.5, isWireframe);
              drawPoly([[-10, -10], [15, -12], [15, 12], [-10, 10]], cArmorMid, cArmorHighlight, 0.5, isWireframe);
              drawPoly([[-5, -2], [10, -3], [12, 5], [-8, 4]], cArmorDark, cArmorHighlight, 0.5, isWireframe);
              drawPoly([[15, -12], [35, -10], [35, 6], [15, 12]], cArmorLight, cArmorHighlight, 0.5, isWireframe);
              drawPoly([[35, -10], [50, -4], [50, 2], [35, 2]], cArmorMid, cArmorHighlight, 0.5, isWireframe);
              drawPoly([[50, -4], [55, -2], [55, 2], [50, 2]], cArmorDark, cArmorHighlight, 0.5, isWireframe);
          };
          
          // 1. Draw wireframe hologram (unbuilt part)
          ctx.save();
          ctx.globalCompositeOperation = 'screen';
          applyGlow(10, '#a855f7');
          drawWhaleParts(true);
          ...
          // 2. Clip region for solid construction
          const buildX = -40 + (p * 100); 
          
          ctx.save();
          ctx.beginPath();
          ctx.rect(-50, -50, buildX + 50, 100);
          ctx.clip();
          
          // Static whale body (solid)
          drawWhaleParts(false);
          
          ctx.restore(); // End clip
          
          // 3. Draw the construction laser line and sparks
          ctx.globalCompositeOperation = 'screen';
          ...
          ctx.lineWidth = 3;
          ctx.beginPath();
          ctx.moveTo(buildX, -20);
          ctx.lineTo(buildX, 20);
          ctx.stroke();
          ...
          for(let i=0; i<8; i++) {
              ...
              const sparkX = buildX + (Math.random()-0.5)*4;
              
              ctx.fillStyle = Math.random() > 0.5 ? '#ffffff' : '#facc15';
              ctx.beginPath();
              ...
              ctx.lineTo(sparkX - Math.random() * 10, sparkY + (Math.random() - 0.5) * 5);
              ctx.stroke();
          }
          
          // Scanning vertical line
          const scanY = -20 + ((timeMs * 0.1) % 40);
          ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
          ctx.fillRect(buildX - 2, scanY, 4, 1);
          
          ctx.restore(); // Restore whale scale
          
          ctx.restore(); // Restore builder translation
      }
      ...
      if (castProgress > 0) {
          // Hat floats up and glows
          const floatUp = Math.sin(castProgress * Math.PI) * 15;
          ctx.translate(0, -floatUp);
          
          // Intense glow during casting
          applyGlow(20 + Math.sin(time * 0.02) * 20, cPurpleGlow);
          
          // Constructing particles/beams from head to hat
          ctx.strokeStyle = cPurpleGlow;
          ctx.lineWidth = 1 + Math.random();
          ctx.beginPath();
          ctx.moveTo(-4, 0);
          ctx.lineTo(-4 + (Math.random() - 0.5) * 4, floatUp);
          ctx.stroke();
          ctx.beginPath();
          ctx.moveTo(4, 0);
          ctx.lineTo(4 + (Math.random() - 0.5) * 4, floatUp);
      ...
      if (castProgress > 0) {
          // Hat splits open
          const split = Math.sin(castProgress * Math.PI) * 8;
          
          // Left half
          ctx.save();
          ctx.translate(-split, 0);
          ctx.rotate(-split * 0.05);
          drawPoly([[-10, -11], [0, -11], [0, -25], [-8, -25]], cArmorMid, cArmorHighlight);
          drawPoly([[-9, -25], [0, -25], [0, -27], [-10, -27]], cArmorDark, cWhite);
          ctx.restore();
          ...
          ctx.translate(split, 0);
          ctx.rotate(split * 0.05);
          drawPoly([[0, -11], [10, -11], [8, -25], [0, -25]], cArmorMid, cArmorHighlight);
          drawPoly([[0, -25], [9, -25], [10, -27], [0, -27]], cArmorDark, cWhite);
          ctx.restore();
          
          // Glowing core inside hat
          ctx.fillStyle = '#facc15';
          ctx.beginPath();
          ctx.arc(0, -18, 4 + Math.random() * 2, 0, Math.PI * 2);
          ctx.fill();
      } else {
          // Crown Base
      ...
      // Shoulder Pad (Floating/Segmented)
      ...
      // Hand (fingers wrapped around grip)
      ...
      // Railgun Energy Core & Accelerator
      ...
      // Plasma Arcs / Lightning when shooting
